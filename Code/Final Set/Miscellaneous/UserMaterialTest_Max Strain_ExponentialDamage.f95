...
!  Can be used to debug and verify the execution of constitutive law prior to applying it to ABAQUS     !
																										!
!  Failure Criterion: (Max Strain)  Damage Criteria: (Exponential Stiffness reduction)                  !                                                                                            		!
																										!	
!  Date: 25.10.2019																						!
																										!	
...
!=====================================================
   ...
   real :: StrainInc(nblock,ndir+nshr), Strain(nblock, ndir+nshr),StrainMax(nblock,ndir+nshr),DDSDDE(ndir+nshr,ndir+nshr)
   ...
   real :: Xt, Xc, Yt, Yc, Zt, Zc, S12, S13, S23, a_t, n_t, a_c, n_c, a_s, n_s
...
!==================================================
    ...
    Yc=0.020  ! Compressive Strength in normal 2-direction
...
!===============================================================================
	...
	! In the expoenential damage implementation, the material properties are
    ! degraded using an exponential decay function once failure occurs
	! The damage variables develop according to the following relation:

   	! Dmg(I,J) = e^(-a_(t,c,s)*StrainD(I,J)

    ! Where : StrainD(I,J) = (Strain(I,J)-Strainmax(I,J))/(n_(t,c,s)*Strainmax(I,J))
 
	! where: e is the exponential function 
    !        a is a constant positive factor which determines how rapid the gradual degredation happen 
    !        n is a constant nonzero positive factor which determines the strain at whihch full degredation occur
    !        (t,c,s) subscript for tension, compression and shear failure modes
    !		 Strainmax is the strain at the onset of damage, evaluated as Strength/Modulus in the repspective direction

    ! Remark: if a(t,c,s) == 0  no damage is applied
  			
    a_t= 8.0 	! Exponential decay rate factor for Tension  
    
    n_t= 1.0 	! Ratio of tensile strain at full damage to strain at onset of damage
    
    a_c= 5.0 	! Exponential decay rate factor for compression

    n_c= 2.0    ! Ratio of compressive strain at full damage to strain at onset of damage

    a_s= 1.0 	! Exponential decay rate factor for Shear 
    
    n_s= 1.0	! Ratio of shear strain at full damage to strain at onset of damage
		...
		DO I= 1,nblock ! Loop through all material points
        	...
        	DO J=1,3   ! Loop through normal stress components 
        		...
        		IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0)) .AND. (Strain(I,J).GE.0.0)) THEN  ! Check for tensile failure
                    ...
                    StrainMax(I,J) = (1.0/e(I,J))*Strain(I,J)

					Dmg(I,J)= EXP(-a_t*(Strain(I,J)- StrainMax(I,J))/(n_t*StrainMax(I,J)))  ! Apply tensile damage 
    			...
    			ELSE IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0)) .AND. (Strain(I,J).LT.0.0)) THEN  ! Check for compressive failure
					...
					Dmg(I,J)= EXP(-a_c*(Strain(I,J)- StrainMax(I,J))/(n_c*StrainMax(I,J)))  ! Apply compressive damage
        	...
        	DO J=4,6   ! Loop through shear stress components 
				...
				IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0))) THEN  ! Check for shear failure
					...
					Dmg(I,J)= EXP(-a_s*(Strain(I,J)- StrainMax(I,J))/(n_s*StrainMax(I,J)))   ! Apply shear damage
...
!================================================================== 
 ...
 Print *, 'Strain Increment in normal 2-direction: ', StrainMax(1,2)  ! print Stain increment (2) for material point number 1
 Print *, '________________________________________________________________'
