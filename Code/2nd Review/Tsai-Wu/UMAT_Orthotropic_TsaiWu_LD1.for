...
C  	USER DEFINED VARAIBLES IN UMAT SUBROUTINE
	  ...
	  DOUBLE PRECISION F1,F2,F3,F11,F22,F33,F44,F55,F66,F12,F13,F23,Phi,Phimax
...
! LIMIT ALL TSAI-WU FAILURE POLYNOMYALS TO 1

        DO I=1,6 
		
		IF(Xphi(I).GT.0.0) THEN
...
! TENSION OR COMPRESSION FAILURE
			...
			DO I=1,3
			 ...
			 IF ((I.EQ.IMAX(I)).AND.(STRESS(I).GE.0.0)) THEN
			    ...
			    dmg(I)=(-Dgrd(1)*Xphi(I))+1.0
				
			ELSE IF((I.EQ.IMAX(I)).AND.(STRESS(I).LT.0.0)) THEN
				
				dmg(I)=(-Dgrd(2)*Xphi(I))+1.0
