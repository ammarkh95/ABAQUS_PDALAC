...
!  Can be used to debug and verify the execution of constitutive law prior to applying it to ABAQUS     !
																										!
!  Failure Criterion: (Tsai-Wu)  Damage Criteria: (Gradual Stiffness reduction)                       !                                                                                            		!
																										!	
!  Date: 25.10.2019																						!
																										!	
...
!=====================================================
   ...
   real :: Xt, Xc, Yt, Yc, Zt, Zc, S12, S13, S23, Xphi(nblock,6)
   real :: F1,F2,F3,F11,F22,F33,F44,F55,F66,F12,F13,F23,Phi(nblock) !Tsai-Wu failure polynomials   
...
!==================================================
    ...
    Yt=0.01  ! Tension Strength in normal 2-direction
...
!====================================================================================================   	
	    ...
	    IF (STEP.EQ.1) THEN
			...
			! Initialize all failure flags and Tsai-Wu failure polynomial
...
!3.5 (Failure Evaluation TSAI-WU)
!==================================================================
...
!==================================================================
        ...
        DO I= 1,nblock ! Loop through all material point and check damage initiation      
         	...
         	IF (Phi(I).GT. 1.0) THEN  ! check if Tsai-Wu failure polynomial excceds one
            	...
            	DO J=1,3 ! Loop through normal stress components 
                	...
                	IF ((J.EQ.IMAX(I)) .AND. (StressNew(I,J).GE.0.0)) THEN  ! Tesnile failure check
						...
						Dmg(I,J)= Dmg(I,J)*(1.0/Phi(I))  ! Apply tensile damage 
                    ...
                    ELSE IF ((J.EQ.IMAX(I)) .AND. (StressNew(I,J).LT.0.0)) THEN  ! Compressive failure check
						...
						Dmg(I,J)= Dmg(I,J)*(1.0/Phi(I))  ! Apply compressive damage
        	    ...
        	    DO J=4,6   ! Loop through shear stress components 
					...
					Dmg(I,J)= Dmg(I,J)*(1.0/Phi(I))  ! Apply shear damage
...
!================================================================== 
 ...
 print '(" Failure ratio in normal 2-direction: ",f6.3)',Phi(1) ! print Tsai-Wu polynomial for material point number 1
...
! The following subroutine : 1. Build the effective constiutive matrix based on orthotropic elasticity
