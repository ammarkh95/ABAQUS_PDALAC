...
!  Can be used to debug and verify the execution of constitutive law prior to applying it to ABAQUS     !
																										!
!  Failure Criterion: (Max Stress)  Damage Criteria: (Recursive Stiffness reduction)                    !                                                                                            		!
																										!	
!  Date: 25.10.2019																						!
																										!	
...
!===============================================================================
    ...
    XbetaT=0.1 ! Tension Stiffness Degredation Factor (scale from 0(no damage) to 1(full damage))
...
!==================================================================
		...
		DO I= 1,nblock ! Loop through all material points
        	...
        	DO J=1,3   ! Loop through normal stress components 
        		...
        		IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0)) .AND. (StressNew(I,J).GE.0.0)) THEN  ! Check for tensile failure Or previous damage
    			...
    			ELSE IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0)) .AND. (StressNew(I,J).LT.0.0)) THEN  ! Check for compressive failure or previous damage
        	...
        	DO J=4,6   ! Loop through shear stress components 
				...
				IF(((e(I,J).GT.1.0).OR.(Dmg(I,J).LT.1.0))) THEN  ! Check for shear failure or previous damage
