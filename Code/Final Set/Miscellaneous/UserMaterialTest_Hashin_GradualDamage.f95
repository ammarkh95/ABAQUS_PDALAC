...
!  Can be used to debug and verify the execution of constitutive law prior to applying it to ABAQUS     !
																										!
!  Failure Criterion: (Hashin)  Damage Criteria: (Gradual Stiffness reduction)                          !                                                                                            		!
																										!	
!  Date: 25.10.2019																						!
																										!	
...
!=====================================================
   ...
   real :: Xt, Xc, Yt, Yc, Zt, Zc, S12, S13, S23
...
!==================================================
    ...
    Yt=100.0  ! Tension Strength in normal 2-direction
...
!$$$$$$    !!!!! Note that no degredation factors input required for gradual damage !!!!!!!
!$$$$$$    !!!!! since damage is governed by yield surface of the failure criteria  !!!!!!!
    
!$$$$$$     XbetaT=0.97 ! Tension Stiffness Degredation Factor
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           IF((e(I,1).GT.1.0) .AND. (StressNew(I,1).GE.0.0)) THEN  ! Check for fiber tensile failure

    				fflags(I,1)=STEP    ! Assign Failure Flag to step number as a positive number

					Dmg(I,1)= Dmg(I,1)*(1.0/e(I,1))  ! Apply fiber tensile damage 
					Dmg(I,4)= Dmg(I,4)*(1.0/e(I,1))  ! Apply shear damage 


    	   ELSE IF((e(I,1).GT.1.0) .AND. (StressNew(I,1).LT.0.0)) THEN  ! Check for fiber compressive failure

    				fflags(I,1)=-STEP    ! Assign Failure Flag to step number as a negarive number

					Dmg(I,1)= Dmg(I,1)*(1.0/e(I,1))  ! Apply fiber compressive damage
           END IF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!           

		   ! Matrix Damage Modes

           IF((e(I,2).GT.1.0) .AND. ((StressNew(I,2)+StressNew(I,3)).GE.0.0)) THEN  ! Check for matrix tensile failure 

    				fflags(I,2)=STEP    ! Assign Failure Flag to step number as a positive number

					Dmg(I,2)= Dmg(I,2)*(1.0/e(I,2))  ! Apply matrix tensile damage 
					Dmg(I,4)= Dmg(I,4)*(1.0/e(I,2))  ! Apply shear damage (1-2)
                    Dmg(I,6)= Dmg(I,6)*(1.0/e(I,2))  ! Apply shear damage (2-3)

    	   ELSE IF((e(I,2).GT.1.0) .AND. ((StressNew(I,2)+StressNew(I,3)).LT.0.0)) THEN  ! Check for matrix compressive failure 

    				fflags(I,2)=-STEP    ! Assign Failure Flag to step number as a negarive number

					Dmg(I,2)= Dmg(I,2)*(1.0/e(I,2))  ! Apply matrix compressive damage
					Dmg(I,4)= Dmg(I,4)*(1.0/e(I,2))  ! Apply shear damage (1-2)
                    Dmg(I,6)= Dmg(I,6)*(1.0/e(I,2))  ! Apply shear damage (2-3)                    
           END IF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!           
           ...
           IF((e(I,3).GT.1.0) .AND. (StressNew(I,3).GE.0.0)) THEN  ! Check for interlaminar tensile failure or previous damage

    				fflags(I,3)=STEP    ! Assign Failure Flag to step number as a positive number

					Dmg(I,3)= Dmg(I,3)*(1.0/e(I,3))  ! Apply tensile damage 


    	   ELSE IF((e(I,3).GT.1.0) .AND. (StressNew(I,3).LT.0.0)) THEN  ! Check for interlaminar compressive failure
           
    				fflags(I,3)=-STEP    ! Assign Failure Flag to step number as a negarive number

					Dmg(I,3)= Dmg(I,3)*(1.0/e(I,3))   ! Apply fiber compressive damage
           END IF
