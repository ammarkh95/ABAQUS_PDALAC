...
C   FIND ACTION PLANE MOST LIKELY TO FAIL BY CALCULATING MAXIMUM FAILURE INDEX fIFF
         FMAX = 0
         DO THETA =-pi/2, pi/2 ,pi/180   !LOOp THROUGH THETA, FIND PLANE WITH MAX FAILURE INDEX
...
! INTERFIBER FAILURE
        IF (fIFF.GE. 1.0) THEN
		    dmg3 = dmg3*(0.999999 -Dgrd)      !DECREASE SHEAR STIFNESS
			IF  (SNF.GE. 0.0) THEN
			    dmg2 =  dmg2*(0.999999 -Dgrd)   !DECREASE LATERAL STIFFNESS IF NORMAL STRESS ON ACTION PLANE IS POSITIVE
			ELSE
			    dmg2 = 1
			END IF
		ELSE 
...
C 	BUILD DEGRADED CONSTITUTIVE TENSOR IN CASE OF FAILURE DETECTION  
      ...
      DDSDDE(1,2)=(E11*Upsilon*(XNU21+XNU31*XNU23))* abs(dmg1)*abs(dmg2)
      DDSDDE(1,3)=(E11*Upsilon*(XNU31+XNU21*XNU32))* abs(dmg1)*abs(dmg2)
      DDSDDE(2,1)=(E11*Upsilon*(XNU21+XNU31*XNU23))* abs(dmg1)*abs(dmg2)
      DDSDDE(2,2)=(E22*Upsilon*(ONE-XNU13*XNU31))*abs(dmg2)
      DDSDDE(2,3)=(E22*Upsilon*(XNU32+XNU12*XNU31))* abs(dmg1)*abs(dmg2)
      DDSDDE(3,1)=(E11*Upsilon*(XNU31+XNU21*XNU32))* abs(dmg1)*abs(dmg2)
      DDSDDE(3,2)=(E22*Upsilon*(XNU32+XNU12*XNU31))* abs(dmg1)*abs(dmg2)
      DDSDDE(3,3)=(E33*Upsilon*(ONE-XNU12*XNU21))*abs(dmg2)
      DDSDDE(4,4)=G12*abs(dmg3)
      DDSDDE(5,5)=G13*abs(dmg3)
      DDSDDE(6,6)=G23*abs(dmg3)
