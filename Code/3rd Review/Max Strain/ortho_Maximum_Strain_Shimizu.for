...
C	------------------------------------------------------------------
	!ultimate strain allowable value
	  eUltT11 = PROPS(10)
	  eUltC11 = PROPS(11)
	  eUltT22 = PROPS(12)
	  eUltC22 = PROPS(13)
	...
	! ultimate stress for element deletion  
...
C	<Maximum strain criteria>
C	------------------------------------------------------------------
	! read and fill ultimate strain(tension, compression, shear) for maximum strain criteria	  
	!Tell whether each stresses is tension or compression
	  if(STRESS1(1).ge.ZERO) then
		eUlt11 = eUltT11
		Beta11 = BetaT11!2-different stiffness
		DamageCoefficient11 = DamageCoefficientT11!3-Recursive approach
	  ...
	  if(STRESS1(2).ge.ZERO) then
		eUlt22 = eUltT22
		Beta22 = BetaT22
		DamageCoefficient11 = DamageCoefficientT22
	  else if(STRESS1(2).lt.ZERO) then
	    eUlt22 = -1.0*eUltC22
		Beta22 = BetaC22
		DamageCoefficient11 = DamageCoefficientC22
	  !else
	  end if
	  ...
	  if(STRESS1(3).ge.ZERO) then
		eUlt33 = eUltT33
		Beta33 = BetaT33
		DamageCoefficient11 = DamageCoefficientT33
	  else if(STRESS1(3).lt.ZERO) then
	    eUlt33 = -1.0*eUltC33
		Beta33 = BetaC33
		DamageCoefficient11 = DamageCoefficientC33
	  !else
	  end if
	  !question: Can shear stress be negative in the simulation?
	  !if so, this code below should be needed:
	  
	  if(STRESS1(4).ge.ZERO) then
		eUlt12 = eUltS12
	  else if(STRESS1(4).lt.ZERO) then
	    eUlt12 = -1.0*eUltS12
	  !else
	  end if
	  Beta12 = BetaS12
	  DamageCoefficient12 = DamageCoefficientS12
	  
	  if(STRESS1(5).ge.ZERO) then
		eUlt13 = eUltS13
	  else if(STRESS1(5).lt.ZERO) then
	    eUlt13 = -1.0*eUltS13
	  !else
	  end if
	  Beta13 = BetaS13
	  DamageCoefficient13 = DamageCoefficientS13
	  if(STRESS1(6).ge.ZERO) then
		eUlt23 = eUltS23
	  else if(STRESS1(6).lt.ZERO) then
	    eUlt23 = -1.0*eUltS23
	  !else
	  end if
	  Beta23 = BetaS23
	  DamageCoefficient23 = DamageCoefficientS23
	  
	! Ratios of strain(from analysis) to ultimate strain(from definition by user)
	  ratioStran11 = STRAN(1)/eUlt11
	  ratioStran22 = STRAN(2)/eUlt22
	  ratioStran33 = STRAN(3)/eUlt33
	  ratioStran12 = STRAN(4)/eUlt12
	  ratioStran13 = STRAN(5)/eUlt13
	  ratioStran23 = STRAN(6)/eUlt23
...
C	------------------------------------------------------------------
	flagForFailureApproach = PROPS(29)
	...
	! 1 - for material which occurs immediate brittle fracture after reaching failure stress 
	  ...
	  if(flagForFailureApproach == 1) then
	    if(ElemDeletion == 0)
		  if(ratioStran11.GE.1.0 .OR.ratioStran22.GE.1.0 .OR.ratioStran33.GE.1.0 .OR.ratioStran12.GE.1.0 .OR.ratioStran13.GE.1.0 .OR.ratioStran23.GE.1.0) then
		    DO I = 1, NTENS
			  STRESS1(I) = 0
			END DO
			ElemDeletion = 1
	...
	! 2 - for material which has another (lower) constant tangent stiffness 
		if(flagForFailureApproach == 2) then

			
		  			
		  if(ratioStran11.GE.1.0) then
			DDSDDE1(1,1) = Beta11 * DDSDDE1(1,1)
			DDSDDE1(1,2) = Beta11 * DDSDDE1(1,2)
			DDSDDE1(1,3) = Beta11 * DDSDDE1(1,3)
			if(ratioStran22.GE.1.0) then
			  DDSDDE1(1,2) = Beta22 * DDSDDE1(1,2)
			endif
			if(ratioStran33.GE.1.0) then
			  DDSDDE1(1,3) = Beta33 * DDSDDE1(1,3)
		    endif
		  endif
		  ...
		  if(ratioStran22.GE.1.0) then
			...
			if(ratioStran11.GE.1.0) then
			  DDSDDE1(2,1) = Beta11 * DDSDDE1(2,1)
			endif
			if(ratioStran33.GE.1.0) then
			  DDSDDE1(2,3) = Beta33 * DDSDDE1(2,3)
		    endif
		  endif
	...
	! 3 - Recursive Degradation
		if(flagForFailureApproach == 3) then
		  ...
		  if(ratioStran11.GE.1.0) then
			...
			if(ratioStran22.GE.1.0) then
			  DDSDDE1(1,2) = Gamma22 * DDSDDE1(1,2)
			endif
			if(ratioStran33.GE.1.0) then
			  DDSDDE1(1,3) = Gamma33 * DDSDDE1(1,3)
		    endif
		  endif
		  ...
		  if(ratioStran22.GE.1.0) then
			...
			if(ratioStran11.GE.1.0) then
			  DDSDDE1(2,1) = Gamma11 * DDSDDE1(2,1)
			endif
			if(ratioStran33.GE.1.0) then
			  DDSDDE1(2,3) = Gamma33 * DDSDDE1(2,3)
		    endif
		  endif
		  ...
		  if(ratioStran22.GE.1.0) then
			...
			if(ratioStran11.GE.1.0) then
			  DDSDDE1(3,1) = Gamma11 * DDSDDE1(3,1)
			endif
			if(ratioStran22.GE.1.0) then
			  DDSDDE1(3,2) = Gamma22 * DDSDDE1(3,2)
		    endif
		  endif
...
C	------------------------------------------------------------------
	...
	<state variables>
C	------------------------------------------------------------------
	! for 3-ply-approach, recent coefficeint of degradation should be preserved
	  STATEV(1) = Gamma11
	  STATEV(2) = Gamma22
	  STATEV(3) = Gamma33
	  STATEV(4) = Gamma12
