...
! !C=========================================================================
        	...
        	DO J=1,3   ! Loop through normal stress components 
        		...
        		IF(((e(J).GT.1.0).OR.(dmg(J).LT.1.0)) .AND. (stressNew(I,J).GE.0.0)) THEN  ! Check for tensile failure

    				fflags(J)=stepTime    ! Assign Failure Flag to step number as a positive number

					dmg(J)= dmg(J)*(0.999999-Dgrd(1))  ! Apply tensile damage 


    			ELSE IF(((e(J).GT.1.0).OR.(dmg(J).LT.1.0)) .AND. (stressNew(I,J).LT.0.0)) THEN  ! Check for compressive failure or previous damage

    				fflags(J)=-stepTime    ! Assign Failure Flag to step number as a positive number

					dmg(J)= dmg(J)*(0.999999-Dgrd(2))  ! Apply compressive damage
        	...
        	DO J=4,6   ! Loop through shear stress components 
				...
				IF(((e(J).GT.1.0).OR.(dmg(J).LT.1.0))) THEN  ! Check for shear failure or previos damage


    				fflags(J)=stepTime    ! Assign Failure Flag to step number as a positive number

					dmg(J)= dmg(J)*(0.999999-Dgrd(3))  ! Apply shear damage
