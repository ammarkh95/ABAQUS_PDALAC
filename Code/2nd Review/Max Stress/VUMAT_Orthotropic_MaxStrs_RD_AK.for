...
C=========================================================================
		...
		DO I= 1,nblock ! Loop through all material points
        	...
        	DO J=1,3   ! Loop through normal stress components 
        		...
        		IF(((e(I,J).GT.1.0).OR.(dmg(I,J).LT.1.0)) .AND. (stressNew(I,J).GE.0.0)) THEN  ! Check for tensile failure

    				fflags(I,J)=totalTime    ! Assign Failure Flag to step number as a positive number

					dmg(I,J)= dmg(I,J)*(0.999999-Dgrd(1))  ! Apply tensile damage 
    			...
    			ELSE IF(((e(I,J).GT.1.0).OR.(dmg(I,J).LT.1.0)) .AND. (stressNew(I,J).LT.0.0)) THEN  ! Check for compressive failure or previous damage
					...
					dmg(I,J)= dmg(I,J)*(0.999999-Dgrd(2))  ! Apply compressive damage
        	...
        	DO J=4,6   ! Loop through shear stress components 
				...
				IF(((e(I,J).GT.1.0).OR.(dmg(I,J).LT.1.0))) THEN  ! Check for shear failure or previos damage


    				fflags(I,J)=totalTime    ! Assign Failure Flag to step number as a positive number

					dmg(I,J)= dmg(I,J)*(0.999999-Dgrd(3))  ! Apply shear damage
